...
// channel/relay.ts is a byte-identical copy shared with openclaw-plugin, same convention as
// auth/pkce-login.ts (pinned from the openclaw side in openclaw-plugin/__tests__). Everything
...
describe('relay identity', () => {
  test('channel/relay.ts is byte-identical to the openclaw-plugin copy', () => {
    const here = import.meta.dirname
    const ours = join(here, '..', 'relay.ts')
    const theirs = join(here, '..', '..', '..', 'openclaw-plugin', 'channel', 'relay.ts')
